...
They collect behind the **☆ X saved** button in the top-right corner of the page, which opens a sidebar down the right-hand side. Each bar is drawn as its own small staff — the same picture the share panel makes, with the tempo written over it as a tempo mark — so you pick one out by its notation rather than by a name. Nothing is written under the notation; when you kept it is along the top of the card instead.
...
Drag a card by the **⠿** on its left to move it up or down the list, by mouse or by finger; the new order is kept.

The **×** removes a bar, and asks first: it turns into **remove?** for a few seconds, and only the second tap removes anything. A message along the bottom of the page then offers **Undo**, and Ctrl+Z (⌘Z on a Mac) does the same thing, putting the bar back where it was.

On a wide enough window the sidebar sits beside the app, which shifts over to make room, so you can keep the list open while you play. It is wider there, and the notation on each card is drawn bigger to match. Anywhere narrower it lies over the page instead, and tapping the page outside it, pressing Escape, or loading a bar closes it again. Where it sits alongside, it is still open when you come back; where it covers the page it always starts closed.
...
- **The tempo dial** sets the beats per minute, from 40 to 200. Turn the dial anywhere on its face to sweep the tempo, tap **‹** and **›** for one beat at a time (hold them to run), or — where there is a keyboard — click the number and type one. The current tempo is on the dial itself and the teal ring shows where it sits in the range. The knurled rim is a real wheel: it lights up when you take hold of it and turns under your finger exactly as far as you turn it, and it sits at the same angle for a given tempo however that tempo was set. A change takes effect immediately: whatever is playing bends to the new tempo without stopping or losing its place, count-in included.
...
- **The time signature** is set on the staff itself. Tap the **4/4** at the start of a line and the meters open beside it: 2/4, 3/4, 4/4, 5/4, 6/4, or 7/4. The quarter note is the beat in all of them, so the tempo means the same thing in every meter. The count-in runs one bar of it and the metronome accents beat 1. Changing it mid-run takes effect on the next rhythm. Until you have tapped it once, a small dot pulses beside the signature to say that it can be tapped; after that it stays out of the way.
- **The loop button** in the top-right corner of the staff repeats the current bar instead of moving on to a new one, and the staff closes on a **repeat sign** while it is on. Retry turns it on for you.
...
- **The two sound rows** — a cowbell for the rhythm and a metronome for the click — each have an icon and a volume slider. Tapping an icon mutes that sound outright and the row dims; moving its slider brings it back. Both are live: they reach sounds that were already scheduled, so a change is audible on the next note. Both scales mean the same thing, so 100 on one is as loud as 100 on the other. The metronome is on by default at 30, so it sits under the cowbell. On a wide enough window each row is labelled — `rhythm`, `metronome` — with the name to the left and the icon beside its own slider.
